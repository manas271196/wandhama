...
var x = 0;
var perRow = 7;
var desc = d3.select(".pic-description"); // desc.append("p").attr("class", "name").text("Lorem Ipsum");
// desc.append("p").attr("class", "age").text("XX years");

for (var i = x; i < x + perRow; i++) {
...
for (var _i = x; _i < x + perRow; _i++) {
  var photos = d3.select("#photos > .row-2");
  var cont = photos.append("div").attr("class", "photo-container");
  cont.append("img").attr("src", "pics/" + pics[_i]);
}

x += perRow;

for (var _i2 = x; _i2 < x + perRow && _i2 < pics.length; _i2++) {
  var photos = d3.select("#photos > .row-3");
  var cont = photos.append("div").attr("class", "photo-container");
  cont.append("img").attr("src", "pics/" + pics[_i2]);
}

pics.forEach(function (pic, i) {});
d3.select(".go-down").on("click", function () {
...
d3.selectAll(".photo-container").on("mouseover", function (e) {
  ...
  desc.select(".age").text(rand[1]); // desc.classed("show", true);

  desc.transition().duration(1000).style("opacity", 1);
});
d3.selectAll(".photo-container").on("mouseout", function (e) {// desc.classed("show", false);
